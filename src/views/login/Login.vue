<template>
  <div class="min-h-screen flex bg-slate-50 font-sans">
    ...
    <div class="hidden lg:flex lg:w-1/2 relative bg-slate-900 overflow-hidden items-center justify-center">
      ...
      <!-- 动态切换的真实背景图 (采用多图层叠 + 透明度交叉淡入淡出，彻底解决切换僵硬问题) -->
      ...
      <!-- 科技感遮罩层：加深了中间区域的压暗程度(via-slate-900/50)，保证白色字体绝对清晰 -->
      ...
      <!-- 为文字容器增加阴影 (drop-shadow-2xl 更强烈的阴影保护文字可读性) -->
    ...
    <div class="w-full lg:w-1/2 flex flex-col justify-center relative bg-white shadow-[-20px_0_40px_rgba(0,0,0,0.05)] z-20">
      ...
      <div class="w-full max-w-sm mx-auto px-6 py-12">
        <div class="mb-10">
          ...
          <p class="text-slate-500 text-sm">请输入 {{ roleConfig.title }} 授权域账号</p>
        </div>
        ...
        <form @submit.prevent="handleLogin" class="space-y-6">
          ...
          <div>
            ...
            <div class="relative">
              ...
              <input
                ...
                required
                class="block w-full pl-10 pr-3 py-3 border border-slate-200 rounded-lg text-slate-900 focus:outline-none focus:ring-1 transition-colors bg-slate-50 focus:bg-white text-sm"
                :class="roleConfig.focusClass"
                placeholder="请输入密码"
              />
            </div>
          ...
          <button
            type="submit"
            :disabled="loading"
            class="w-full flex justify-center items-center gap-2 py-3.5 border border-transparent rounded-lg shadow-sm text-sm font-bold text-white transition-all duration-500 mt-4 hover:shadow-md"
        ...
        <div class="mt-10 pt-6 border-t border-slate-100">
          <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-3">Dev Env Quick Login</p>
...
import { User, Lock, ArrowLeft, ShieldCheck, Activity, AlertOctagon, Waves, LineChart, Cpu } from 'lucide-vue-next'
...
// 核心亮点：根据不同角色，动态切换文案、图标、主题色彩
const roleConfigs = {
  farmer: { 
    title: '生产与示范端', 
    desc: '执行基地日常巡塘录入、苗种出入库调拨与边缘设备调控。',
    icon: Waves,
    textClass: 'text-teal-500', // 稍微调亮了文本色提升对比
    bgClass: 'bg-teal-500',
  ...
  regulator: { 
    title: '科研与监管端', 
    desc: '宏观把控区域养殖态势，进行全链路合规审计与生长数据挖掘。',
    icon: LineChart,
    textClass: 'text-blue-400', // 稍微调亮了蓝色的文本色提升对比
    bgClass: 'bg-blue-500',
  ...
  admin: { 
    title: '中枢与运维端', 
    desc: '执行系统底层生物学阈值字典调优与区域账号权限指派。',
    icon: Cpu,
    textClass: 'text-slate-400',
    bgClass: 'bg-slate-500',
    lightBgClass: 'bg-slate-100',
    focusClass: 'focus:border-slate-500 focus:ring-slate-500',
...
const handleLogin = async () => {
  ...
  await new Promise(resolve => setTimeout(resolve, 800)) // 模拟真实加密校验耗时

  if (loginForm.password !== '123456') {
    errorMsg.value = '您输入的密码有误，请重新核对'
    loading.value = false
    return
  }
  ...
  if (currentRole.value === 'farmer' && loginForm.username === 'farmer') {
    userConfig = { role: 'FARMER', name: '华农第一产学研基地' }
    path = '/dashboard/farmer'
  } else if (currentRole.value === 'regulator' && loginForm.username === 'leader') {
    userConfig = { role: 'REGULATOR', name: '海洋学院数据中心' }
    path = '/dashboard/regulator'
  } else if (currentRole.value === 'admin' && loginForm.username === 'admin') {
    userConfig = { role: 'ADMIN', name: '系统总干事' }
    path = '/dashboard/admin'
  } else {
    errorMsg.value = '鉴权拒绝：该角色域下无此账号操作权限'
    loading.value = false
    return
  }

  localStorage.setItem('aqua_token', 'mock_token_123')
  localStorage.setItem('aqua_user', JSON.stringify(userConfig))
