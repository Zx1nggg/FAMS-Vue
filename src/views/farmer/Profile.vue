<template>
  <div class="max-w-4xl mx-auto w-full pb-10">
    <div class="mb-6">
      <h1 class="text-2xl font-bold text-gray-800">个人主页</h1>
      <p class="text-sm text-gray-500 mt-1">展示您的对外社交名片与基础资料。</p>
    </div>
        ...
        <section>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="bg-gray-50/70 border border-gray-100 p-4 rounded-xl flex items-center justify-between">
              <div>
                <p class="text-xs text-gray-500 mb-1">登录账号 (手机号)</p>
    ...
    <!-- ==================== 4. 主页编辑专属弹窗 (仅修改昵称等不敏感信息) ==================== -->
    <el-dialog v-model="dialogVisible" title="编辑个人资料" width="450px" append-to-body class="!rounded-2xl" @close="resetForm">
      <div class="px-2 pt-2">
        <el-form ref="editFormRef" :model="editForm" :rules="rules" label-position="top" class="pr-2">
          
          <el-form-item label="网名 / 昵称" prop="username">
            <el-input v-model="editForm.username" placeholder="请输入对外的展示昵称" clearable />
          </el-form-item>
        ...
        <div class="mt-4 text-xs text-amber-600 bg-amber-50 p-2.5 rounded-lg border border-amber-100 flex gap-2">
          <AlertTriangle class="w-4 h-4 shrink-0" />
          注：真实姓名、手机号、电子邮箱等核心实名信息，请前往【账号设置】页面中进行修改。
        </div>
      </div>
...
import { 
  ...
  CheckCircle2, UserSquare, Phone, Mail, MapPin, RefreshCw, AlertTriangle
...
// 页面展示的主数据源
const form = ref({
  username: '', // 现在的 username = 网名/昵称
  realName: '',
  phone: '',    // 现在的 phone = 唯一登录凭证
  email: '',
  gender: 0,
  address: '',
  avatar: '',
...
const rules = {
  username: [
    { required: true, message: '网名/昵称不能为空', trigger: 'blur' },
...
const userInitial = computed(() => {
  return form.value.username ? form.value.username.charAt(0) : '?'
})
